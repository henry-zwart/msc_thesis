...
def plot_corr_with_dendro(
    ...
    if no_cbar:
        g = sns.clustermap(
            ...
            center=0,
            cmap=DIVERGING_CMAP,
            row_colours=y_category_colours,
            col_colours=x_category_colours,
            vmin=-1,
            vmax=1,
    ...
    g.axe_heatmap.set_xticks(
        np.arange(len(x_labels)) + 0.5,
        x_labels,
        rotation=45,
        horizontalalignment="right",
    )
    g.axe_heatmap.set_yticks(np.arange(len(y_labels)) + 0.5, y_labels, rotation=0)
