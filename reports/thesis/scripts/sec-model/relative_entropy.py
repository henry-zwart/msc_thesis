...
def main(P: npt.NDArray[np.float64], rng: np.random.Generator):
    M, _, N = P.shape
    K = 5
    ...
    # Plot relative entropy distribution across individuals
    ...
    for ext in ("png", "pdf"):
        fig.savefig(
            f"reports/thesis/results/figures/model_fit/cross_validation_relative_entropy.{ext}",
            bbox_inches="tight",
        )

    # Plot highest relative entropy on eval sets
    ...
    top_8_idxes = np.argsort(all_kl_results_eval.mean(axis=-1))[-8:]
    for ax, cv_idx in zip(axes.flatten(), top_8_idxes, strict=True):
        ...
        for i in range(N):
            if np.abs(P[idx, 0, i] - P[idx, 1, i]) < 0.15:
                continue
    ...
    for ext in ("png", "pdf"):
        fig.savefig(
            f"reports/thesis/results/figures/model_fit/high_relative_entropy.{ext}",
            bbox_inches="tight",
        )
