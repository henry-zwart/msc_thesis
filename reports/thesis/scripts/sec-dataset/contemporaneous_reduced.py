...
def main(df: pl.DataFrame, labels: list[str]):
    contemp = Correlation.VAR_CONTEMPORANEOUS.calculate(
        df, assume_centered=True, regularised=True
    )
    _linkage = linkage(squareform(1 - np.abs(contemp), checks=False), method="average")
    idx_order = leaves_list(_linkage)
    contemp = contemp[idx_order][:, idx_order]
    labels = np.asarray(labels)[idx_order]
    mask = np.triu(np.ones_like(contemp, dtype=np.bool))
    mask[abs(contemp) < 0.05] = 1
    ...
    fig.savefig(
        "reports/thesis/results/figures/dataset/reduced_subset_contemporaneous.pdf",
        bbox_inches="tight",
    )
