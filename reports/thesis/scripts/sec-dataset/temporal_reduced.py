...
def main(df: pl.DataFrame, labels: list[str]):
    temporal = Correlation.VAR_TEMPORAL.calculate(
        df, assume_centered=True, regularised=True
    )
    _linkage = linkage(squareform(1 - np.abs(temporal), checks=False), method="average")
    idx_order = leaves_list(_linkage)
    temporal = temporal[idx_order][:, idx_order]
    labels = np.asarray(labels)[idx_order]
    mask = np.zeros_like(temporal, dtype=np.bool)
    mask[abs(temporal) < 0.05] = 1
    ...
    axes[0].set_yticks(np.arange(len(labels)) + 0.5, labels, rotation=0)
    ...
    fig.savefig(
        "reports/thesis/results/figures/dataset/reduced_subset_temporal.pdf",
        bbox_inches="tight",
    )
