...
def main(P: npt.NDArray[np.float64], λ: float):
    ...
    for k in range(K):
        ...
        kl_results_train.append(
            expected_excess_sampling_surprise(P_train, cv_model).mean()
        )
        kl_results_eval.append(
            expected_excess_sampling_surprise(P_eval, cv_model).mean()
        )
    ...
    plot_data = pl.DataFrame(
        {
            ...
            + ["Self-interactions"] * kl_results_train_null.size
            + ["Full connectivity"] * kl_results_eval.size
            + ["Self-interactions"] * kl_results_eval_null.size,
        }
    )
    ...
    plot_data = pl.DataFrame(
        {
            ...
            "Split": ["Calibration"] * diffs_train.size
            + ["Validation"] * diffs_eval.size,
        }
    )
    ...
    axes[0].set_ylabel("Mean relative entropy\n(bits/spin)", fontsize=10)
    axes[1].set_ylabel("Mean difference in\nrelative entropy", fontsize=10)

    for ext in ("png", "pdf"):
        fig.savefig(
            f"reports/thesis/results/figures/model_fit/mean_relative_entropy.{ext}",
            bbox_inches="tight",
        )
