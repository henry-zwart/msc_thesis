...
The Causal Attitude Network (CAN) model represents an attitude as an
Ising-style network of beliefs (in the inclusive sense defined above), or _spins_,
which are related via bidirectional edges reflecting reinforcing ($+$) or
...
inclusion of self-interaction effects allows us to capture the timescale on which
...
responds to the states of all other beliefs (including itself). As
...
#let factor-2-footnote = footnote[
  The factor of two here simplifies our derivations of the likelihood and partial
  derivatives in the next chapter. Note that this form of the probability is equivalent
  to one in which the factor of two is absorbed into the baseline activation and
  interaction parameters.
]
We then define the conditional probability that $S_i$ takes on the value $+1$ at
time $t+1$ as the logistic transform of the effective baseline activation:#factor-2-footnote
...
Finally, using Definition @eqn:methods-belief-system-dynamics-parts-whole-relation[],
...
All beliefs can update during every time interval. This
update routine is referred to as *synchronous Glauber dynamics*, in contrast with
...
estimation. For an extended discussion of this matter, we refer the reader to
